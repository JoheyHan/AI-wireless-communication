...
def Encoder(x,feedback_bits):
    ...
    with tf.compat.v1.variable_scope('Encoder'):
        for _ in range(6):
            x = layers.Conv2D(20, 8, padding = 'SAME',activation="relu")(x)
        x = layers.Flatten()(x)
...
def Decoder(x,feedback_bits):
    ...
    x = layers.Dense(1024*64, activation='linear')(x)
    x_ini = layers.Reshape((16, 32, 2*64))(x)

    for i in range(6):
        x = layers.Conv2D(512, 3, padding = 'SAME',activation="relu")(x_ini)
